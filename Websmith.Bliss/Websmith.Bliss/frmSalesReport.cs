...
namespace Websmith.Bliss
{
    public partial class frmSalesReport : Form
    {
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            List<ENT.OrderBook> lstENTOrder = this.SearchOrder();
            if (txtCustName.Text.Trim() != "")
            {
                lstENTOrder = lstENTOrder.Where(cust => cust.Name.ToLower().StartsWith(txtCustName.Text.ToLower())).ToList();
            }
            txtTotal.Text = "0";
            dgvSales.Rows.Clear();
            ...
            {
                dgvSales.Rows.Add();
                dgvSales.Rows[i].Cells["OrderNo"].Value = lstENTOrder[i].OrderNo;
                dgvSales.Rows[i].Cells["OrderDate"].Value = lstENTOrder[i].OrderDate;
                dgvSales.Rows[i].Cells["CustName"].Value = lstENTOrder[i].Name;
                dgvSales.Rows[i].Cells["DeliveryTypeName"].Value = lstENTOrder[i].DeliveryTypeName;
                dgvSales.Rows[i].Cells["PaymentMethod"].Value = lstENTOrder[i].PaymentMethod;
                dgvSales.Rows[i].Cells["SubTotal"].Value = lstENTOrder[i].SubTotal;
                dgvSales.Rows[i].Cells["ExtraCharge"].Value = lstENTOrder[i].ExtraCharge;
                dgvSales.Rows[i].Cells["SGSTAmount"].Value = lstENTOrder[i].SGSTAmount;
                dgvSales.Rows[i].Cells["CGSTAmount"].Value = lstENTOrder[i].CGSTAmount;
                dgvSales.Rows[i].Cells["Discount"].Value = lstENTOrder[i].Discount;
                dgvSales.Rows[i].Cells["TipGratuity"].Value = lstENTOrder[i].TipGratuity;
                dgvSales.Rows[i].Cells["PayableAmount"].Value = lstENTOrder[i].PayableAmount;
                txtTotal.Text = Convert.ToString(Convert.ToDecimal(txtTotal.Text) + lstENTOrder[i].PayableAmount);
            }            
        }
