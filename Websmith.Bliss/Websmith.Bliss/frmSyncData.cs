    ...
    public partial class frmSyncData : Form
    {
                ...
                using (Stream dataStream = tRequest.GetRequestStream())
                {
                    ...
                    using (WebResponse tResponse = tRequest.GetResponse())
                    {
                        using (Stream dataStreamResponse = tResponse.GetResponseStream())
                        {
                            ...
                            {
                                responseFromFirebaseServer = tReader.ReadToEnd();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection conn = DAL.GetConnection.GetDBConnection(); // new SqlConnection("server=COM-PC;database=BLISS;uid=sa;pwd=abcd");
                foreach (DataTable dt in ds.Tables)
                {
                    ...
                    {
                        lines = lines + "Error In " + dt.TableName.ToString() + " => " + ex.Message.ToString() + "\r\n";
                    }
                    finally
                    {
                        conn.Close();
        ...
        private void btnSyncData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objLoginDetail.Username = "dev9@websmithsolution.com"; //"salpizza@mail.com";
                objLoginDetail.Password = "12345";
                objLoginDetail.App_Version = "";
                objLoginDetail.Login_Via = "";
                objLoginDetail.Device_ID = "";
                objLoginDetail.IMEI_No = "";
                string objResult = SendBranchAuthentication(objLoginDetail);
                //XmlDocument xml = JsonConvert.DeserializeXmlNode(System.IO.File.ReadAllText(Application.StartupPath + "\\JSONData.txt"), "RootObject");
                XmlDocument xml = JsonConvert.DeserializeXmlNode(objResult, "RootObject");
                DataSet ds = new DataSet("Json Data");
                XmlReader xr = new XmlNodeReader(xml);
                ds.ReadXml(xr);
                int i = 0;
                ...
                foreach (DataTable dt in ds.Tables)
                {
                    try
                    {
                        lblUpdate.Text = "Updating Data In => " + dt.TableName.ToString() + "...";
                        ...
                        switch (dt.TableName.ToString())
                        {
                            ...
                            case "BranchSettingDetail":
                                ...
                                for (int n = 0; n < dt.Rows.Count; n++)
                                {
                                    if (objDALBranchSetting.getCoutBranchSettingByBranchID(GlobalVariable.BranchID) > 0)
                                    ...
                                    objENTBranchSetting.BranchID = new Guid(GlobalVariable.BranchID);
                            ...
                            case "EmployeeMasterList":
                                ...
                                for (int n = 0; n < dt.Rows.Count; n++)
                                {
                                    ...
                                    //objENTEmployee.EmployeeMasterList_Id = Convert.ToInt32(dt.Rows[n]["EmployeeMasterList_Id"]);
                            ...
                            case "EmployeeShift":
                                ...
                                for (int n = 0; n < dt.Rows.Count; n++)
                                {
                                    if (objDALEmpShift.getDuplicateEmployeeShiftByEmpID(GlobalVariable.EmployeeID) > 0)
                                    { objENTEmpShift.Mode = "UPDATE"; }
                                    else
                                    { objENTEmpShift.Mode = "ADD"; }
                                    objENTEmpShift.ShiftID = new Guid(dt.Rows[n]["ShiftID"].ToString());
                                    objENTEmpShift.EmployeeID = new Guid(GlobalVariable.EmployeeID);
                            ...
                            case "ModuleAppIDDetail":
                                ...
                                for (int n = 0; n < dt.Rows.Count; n++)
                                {
                                    //if (objDALApp.getDuplicateModuleAppIDDetailByID(dt.Rows[n]["AppID"].ToString()) > 0)
                                    //{ objENTApp.Mode = "UPDATE"; }
                                    //else
                                    //{ objENTApp.Mode = "ADD"; }
                                    if (n == 0)
                                    {
                            ...
                            case "TableMasterDetail":
                                ...
                                for (int n = 0; n < dt.Rows.Count; n++)
                                {
                                    ...
                                    objENTTable.StatusID = Convert.ToInt32(GlobalVariable.TableStatus.Vacant);
        ...
        private void frmSyncData_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
